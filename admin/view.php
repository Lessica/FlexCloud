...
if ($view_type == "patches") {
	$row = mysql_fetch_assoc(mysql_query("SELECT * FROM `patches` WHERE `ID` = '" . $view_id . "'",$con));
}
elseif ($view_type == "users") {
	$row = mysql_fetch_assoc(mysql_query("SELECT * FROM `users` WHERE `ID` = '" . $view_id . "'",$con));
}
...
<pre>
<?php
    print_r (htmlspecialchars(var_dump($row)));
?>
</pre>
</p>
</div>
