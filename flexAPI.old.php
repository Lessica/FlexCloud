...
if (!$con) {
	$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"无法连接数据库，请联系管理员。"));
	goto endlabel;
  }
...
if ($obj == null) {
	$result = array("status"=>"error","result"=>array("title"=>"请求错误","message"=>"错误的查询请求！"));
	goto endlabel;
}
...
if ($action == "applist") {
	...
	if (strlen($UDID) != 40) {
		$result = array("status"=>"error","result"=>array("title"=>"请求错误","message"=>"错误的参数：未知设备。"));
		goto endlabel;
	}
	...
	$i = 0;
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"读取应用列表失败：applist，请联系管理员。"));
		goto endlabel;
	}
...
elseif ($action == "appPatchList") {
	...
	if ($appID <= 0 or strlen($UDID) != 40) {
		$result = array("status"=>"error","result"=>array("title"=>"请求错误","message"=>"错误的参数：未知设备或应用编号。"));
		goto endlabel;
	}
	
	$sql = "SELECT `ID`, `Identifier` FROM `applist` WHERE `ID` = '" . $appID . "' LIMIT 1";
	$query = mysql_query($sql,$con);
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"读取应用列表失败：applist，请联系管理员。"));
		goto endlabel;
	...
	$sql = "SELECT `ID`, `Name`, `Description`, `Author`, `DownloadTimes`, `iOS`, `Identifier`, `ToShow`, `UUID`, `AuthorID`, `AverageRating` FROM `patches` WHERE `Identifier` = '" . $row[1] . "' and (`ToShow` = '1' or `ToShow` = '9')";
	goto MakePatchList;
}
...
elseif ($action == "patchInfo") {
	...
	if ($patchID <= 0 or strlen($UDID) != 40) {
		$result = array("status"=>"error","result"=>array("title"=>"请求错误","message"=>"错误的参数：未知设备或补丁编号。"));
		goto endlabel;
	}
	...
	$query = mysql_query($sql,$con);
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"读取评分列表失败：rates，请联系管理员。"));
		goto endlabel;
	}
	$row = mysql_fetch_row($query);
	if ($row != false) {
	...
	$query = mysql_query($sql,$con);
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"读取补丁列表失败：patches，请联系管理员。"));
		goto endlabel;
	}
	$row = mysql_fetch_row($query);
	if ($row != false) {
		...
		$result = array("status"=>"error","result"=>array("title"=>"读取失败","message"=>"找不到指定记录。"));
	}
}
...
elseif ($action == "downloadPatch") {
	...
	if ($F_Purchase == true) {
		...
		$query = mysql_query($sql,$con);
		if ($query == false) {
			$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"读取授权列表失败：purchase，请联系管理员。"));
			goto endlabel;
		}
		$row = mysql_fetch_row($query);
		if ($row == false) {
			$result = array("status"=>"error","result"=>array("title"=>"授权验证失败","message"=>"您尚未取得云端下载权限，请申请或购买后再执行该操作。","link"=>$F_Purchase_URL,"linkButton"=>"购买"));
			goto endlabel;
		}
	}
	
	// Get Patch Info
	$userVersion = mysql_real_escape_string($obj->{'version'});
	$sql = "SELECT `ID`, `Name`, `Identifier`, `UUID`, `Description`, `Author`, `Units`, `Version` FROM `patches` WHERE `ID` = '" . $patchID . "' LIMIT 1";
	$query = mysql_query($sql,$con);
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"读取补丁列表失败：patches，请联系管理员。"));
		goto endlabel;
	...
	if ($row != false) {
		// Get Patch Contents
		if ((double)$row[7] >= 1.9) {
			$result = array("status"=>"error","result"=>array("title"=>"下载失败","message"=>"该补丁是使用 Flex " . $row[7] . " 制作的，不兼容您的当前版本。"));
			goto endlabel;
		}
		if (strlen($row[6]) > 0) {
			$units = $row[6];
		...
		if ($UnitsExist == true) {
			$t = time();
			$arr = array("label"=>$row[1],"identifier"=>$row[2],"uuid"=>$row[3],"downloadDate"=>(int)$t,"description"=>$row[4],"author"=>$row[5],"enabled"=>false,"units"=>array("%units%"));
			$patch = array("patch"=>$arr);
			$result = array("status"=>"success","result"=>$patch);
			$replace = true;
			// Add Download Times
		...
		$result = array("status"=>"error","result"=>array("title"=>"文件系统错误","message"=>"找不到指定文件。"));
		}
	}
	else {
		$result = array("status"=>"error","result"=>array("title"=>"读取失败","message"=>"找不到指定记录。"));
...
elseif ($action == "ratePatch") {
	...
	if ($row != false) {
		...
		$query = mysql_query($sql,$con);
		if ($query == false) {
			$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"添加评分失败：rates，请联系管理员。"));
			goto endlabel;
		}
	}
...
elseif ($action == "login") {
	...
	if (strlen($Username) < 1 or strlen($Password) < 7) {
		$result = array("status"=>"error","result"=>array("title"=>"请求错误","message"=>"错误的参数：用户名或密码。"));
		goto endlabel;
	}
	...
	$query = mysql_query($sql,$con);
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"读取用户列表失败：users，请联系管理员。"));
		goto endlabel;
	}
	$row = mysql_fetch_row($query);
	$Account = (int)$row[3];
	if ($row != false) {
		if ($Account == -1) {
			$result = array("status"=>"error","result"=>array("title"=>"登录失败","message"=>"您的注册信息正在审核中，请稍后再试。"));
		}
		elseif ($Account == -2) {
			$result = array("status"=>"error","result"=>array("title"=>"登录失败","message"=>"您的账号存在安全问题，已被系统冻结，请联系管理员解锁。"));
		}
		else {
			$t = time();
		...
		$result = array("status"=>"error","result"=>array("title"=>"登录失败","message"=>"用户名或密码错误，请重试。"));
	}
}
...
elseif ($action == "register") {
	...
	if (strlen($Username) < 4 OR strlen($Password) < 7 OR strlen($Email) <= 0) {
		$result = array("status"=>"error","result"=>array("title"=>"注册失败","message"=>"注册信息限制：用户名不得少于 4 个字符，密码不得少于 7 个字符，电子邮箱不得为空，请重试。"));
		goto endlabel;
	}
	...
	$row = mysql_fetch_row($query);
	if ($row != false) {
		$result = array("status"=>"error","result"=>array("title"=>"注册失败","message"=>"注册信息检查：用户名或邮箱已被占用，请重试。"));
		goto endlabel;
	}
	else {
		goto addnew;
	...
	$query = mysql_query($sql,$con);
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"添加用户失败：users，请联系管理员。"));
		goto endlabel;
	}
	...
	if ($G_RegCheck == 0) {
		...
		$result = array("status"=>"error","result"=>array("title"=>"注册成功","message"=>"您的注册信息已经提交，请等待审核通过。"));
	}
}
...
elseif ($action == "submitPatch") {
	// Check Content
	$Identifier = stripslashes((string)$obj->{"patch"}->{"identifier"});
	$appName = stripslashes((string)$obj->{"patch"}->{"appName"});
	$description = (string)$obj->{"description"};
	
	if (strlen($Identifier) <= 4 or strlen($appName) == 0 or strlen($description) <= 14) {
		$result = array("status"=>"error","result"=>array("title"=>"提交失败","message"=>"提交信息限制：补丁描述不得少于 14 个字符，应用标识符不得少于 4 个字符，应用名称不得为空，请重试。"));
		goto endlabel;
	}
	...
	if ($row == false) {
		...
		$query = mysql_query($sql,$con);
		if ($query == false) {
			$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"添加应用失败：applist，请联系管理员。"));
			goto endlabel;
		}
	}
	...
	$row = mysql_fetch_row($query);
	if ($row == false) {
		$result = array("status"=>"logout","result"=>array("title"=>"提交失败","message"=>"您必须重新登录以进行该操作。"));
		goto endlabel;
	}
	else {
		// Check UUID
		...
		if ($row != false) {
			if ($row[3] == $ThisUserID or $row[1] == $ThisUser) {
				...
				$result = array("status"=>"error","result"=>array("title"=>"提交失败","message"=>"原补丁名：" . $row[0] . "\n原作者：" . $row[1] . "\n上传时间：" . $row[6] . "\n补丁已经被原作者提交，请勿重复操作。"));
				goto endlabel;
			}
		}
		else {
	...
	NewSubmit:
	$SubName = mysql_real_escape_string(stripslashes((string)$obj->{"patch"}->{"label"}));
	...
	$SubUnits = mysql_real_escape_string(json_encode($obj->{"patch"}->{"units"}[0]));
	$SubappID = 0;
	...
	$sql = "INSERT INTO `patches`(`Name`, `Author`, `AuthorID`, `Description`, `Version`, `iOS`, `Identifier`, `UUID`, `ToShow`, `Control`, `Units`, `DownloadTimes`,`UploadStamp`,`appVersion`,`appID`) VALUES('" . $SubName . "','" . $SubAuthor . "','" . $SubAuthorID . "','" . $SubDescription . "','" . $SubVersion . "','" . $SubiOS . "','" . $SubIdentifier . "','" . $SubUUID . "','0','" . $Mark . "','" . $SubUnits . "','0','" . date('Y-m-d H:i:s',$SubTime) . "','" . $SubappVersion . "','" . $SubappID . "')";
	
	$query = mysql_query($sql,$con);
	if ($query == false) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"添加新补丁项目失败：patches，请联系管理员。"));
		goto endlabel;
	}
	
	$InsertID = mysql_insert_id($con);
	if ($InsertID == 0) {
		$result = array("status"=>"error","result"=>array("title"=>"数据库错误","message"=>"获取新补丁自增编号失败：patches，请联系管理员。"));
		goto endlabel;
	}
	...
	if ($FileSize > 0 or $G_Units == false) {
		...
		$result = array("status"=>"error","result"=>array("title"=>"文件系统错误","message"=>"补丁数据文件写入失败，请联系管理员。"));
	...
	if ($State != 1 and $State != 2) {
		$result = array("status"=>"error","result"=>array("title"=>"更新失败","message"=>"该补丁的上一条更新请求（" . $ReadableTime . "）尚未处理完成，请耐心等待。"));
		goto endlabel;
	}
...
elseif ($action == "appTopList") {
	...
	if ($type == "recent") {
		$Order = "ID";
		$sql = "SELECT `ID`, `Name`, `Description`, `Author`, `DownloadTimes`, `iOS`, `Identifier`, `ToShow`, `UUID`, `AuthorID`, `AverageRating` FROM `patches` WHERE `ToShow` = '1' ORDER BY `" . $Order . "` DESC LIMIT " . (string)$G_RankNum;
	}
	elseif ($type == "popular") {
		$Order = "DownloadTimes";
		$sql = "SELECT `ID`, `Name`, `Description`, `Author`, `DownloadTimes`, `iOS`, `Identifier`, `ToShow`, `UUID`, `AuthorID`, `AverageRating` FROM `patches` WHERE `ToShow` = '1' ORDER BY `" . $Order . "` DESC LIMIT " . (string)$G_RankNum;
	...
	while ($row = mysql_fetch_row($query)) {
		...
		*/
		...
		$arr[$i] = array("id"=>(int)$row[0],"label"=>$row[1],"enabled"=>false,"app"=>(int)$appID,"description"=>$row[2],"author"=>$row[3],"authorID"=>(int)$row[9],"downloads"=>(int)$row[4],"identifier"=>$row[6],"uuid"=>$row[8],"averageRating"=>(double)$row[10],"userRating"=>(double)$UserRate,"downloadDate"=>$t);
		$i++;
	}
	if (count($arr) > 0) {
		$arr = array_values($arr);
	...
	$result = array("status"=>"error","result"=>array("title"=>"请求失败","message"=>"非法请求！"));
	goto endlabel;
}
...
if ($result != null) {
	...
	$result = array("status"=>"error","result"=>array("title"=>"请求失败","message"=>"非法操作！"));
	goto endlabel;
}
